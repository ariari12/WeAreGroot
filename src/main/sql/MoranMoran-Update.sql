...
-- 탈퇴사유
DROP TABLE IF EXISTS `MORANMORAN`.`SECESSION` RESTRICT;
-- 회원
...
-- 탈퇴사유
CREATE TABLE `MORANMORAN`.`SECESSION` (
	`m_id`      INT          NULL, -- 회원번호
	`se_reason` VARCHAR(255) NULL, -- 회원탈퇴사유
	`se_date`   DATE         NULL,  -- 회원탈퇴날짜
    FOREIGN KEY (`m_id`) REFERENCES `MORANMORAN`.`MEMBER` (`m_id`) ON DELETE CASCADE
);
...
CREATE TABLE `MORANMORAN`.`COMMENT` (
    ...
    `c_is_maintain` 	INT NULL, -- 정보상태
    `c_retention_period`  	DATE NULL, -- 정보보관기간
    PRIMARY KEY (`c_id`),
    FOREIGN KEY (`m_id`) REFERENCES `MORANMORAN`.`MEMBER` (`m_id`) ON DELETE CASCADE,

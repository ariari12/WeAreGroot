			...
			<div class="frame-8">
				<div class="container">
					...
					<table class="table">
						...
						<tr>
							<td><img
								src="./resources/img/board/${boardVO.get('biImg') }"
								onerror="this.style.display='none'" alt='' /></td>
						</tr>
						<tr>
							<td colspan="4"><c:set var="bId"
									value="${boardVO.get('bId') }"></c:set>
								<button type="button"
									onclick="location.href='<c:url value='/board?cmd=kh' />'">노하우
									목록</button> <!-- 아이디와 작성자가 같은 경우 해당 버튼 생성 --> <c:choose>
									<c:when test="${boardVO.get('mId') eq loginId }">
										<button type="button"
											onclick="location.href='<c:url value='/board?cmd=khModify&bId=${bId}' />'">수정하기</button>
										<button type="button"
											onclick="location.href='<c:url value='/board?cmd=khDelete&bId=${bId}' />'">삭제하기</button>
									</c:when>
									...
									<c:when test="${not empty loginId }">
										...
										<c:choose>
											<c:when test="${likeResult eq 'ok' }">
												<button type="button"
													onclick="location.href='<c:url value='/board?cmd=khModifyLikeCnt&bId=${bId}&likeResult=likeOk' />'">좋아요</button>
											</c:when>
											<c:otherwise>
												<button type="button"
													onclick="location.href='<c:url value='/board?cmd=khModifyLikeCnt&bId=${bId}&likeResult=likeCancel' />'">좋아요취소</button>
											</c:otherwise>
										</c:choose>
									</c:when>
					...
					<table class="table">
						...
						<c:forEach var="commentVO" items="${commentList}">
							...
							<tr id="comment-${commentVO.get('cId')}">
								<td>내용 : ${commentVO.get('contents')} | 작성자 :
									${commentVO.get('nick')} | 좋아요 : ${commentVO.get('likeCnt') }</td>
								<!-- 아이디와 댓글 작성자가 같은 경우 해당 버튼 생성 -->
								<c:choose>
									<c:when test="${commentVO.get('mId') eq loginId }">
										<td>
											<button type="button"
												onclick="location.href='<c:url value='/comment?cmd=commentDelete&cId=${cId}&bId=${bId}&type=${type}' />'">댓글삭제하기</button>
										</td>


										<td>
											<button type="button"
												onclick="showEditForm('${commentVO.get('cId')}', '${commentVO.get('contents')}', '${bId}', '${type}')">댓글수정하기</button>
										</td>
									</c:when>
									...
									<c:when test="${not empty loginId }">
										<td>
											<button type="button"
												onclick="location.href='<c:url value='/comment?cmd=commentModifyLikeCnt&cId=${cId}&bId=${bId}&type=${type}' />'">좋아요</button>
										</td>
									</c:when>
								</c:choose>
							</tr>
					...
					<c:choose>
						<c:when test="${not empty loginId }">
							<form action="comment" method="get">
								<h3>댓글작성</h3>
								<input type="hidden" name="cmd" value="commentWrite"> <input
									type="hidden" name="bId" value="${boardVO.get('bId')}">
								<input type="hidden" name="mId" value="${loginId}"> <input
									type="hidden" name="type" value="${boardVO.get('type')}">
